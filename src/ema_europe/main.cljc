(ns ema-europe.main
  "Kotodama WASM entrypoint for the ema_europe clean-room actor (L4) — Clojure port.
  ...
  facts (`ema_europe.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ema_europe")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ema_europe.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ema_europe-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ema_europe-compat/ready)
